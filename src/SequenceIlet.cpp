...
* @brief Constructor
**/
SequenceIlet::SequenceIlet(Clock * _clk_instance, ManyCoreArch * _manycore_ptr)
{
    this->seq_type = RANDOM;
    this->init(_clk_instance, _manycore_ptr);
}

SequenceIlet::SequenceIlet(Sequence_Type _seq_type, Clock * _clk_instance, ManyCoreArch * _manycore_ptr)
{
...
void SequenceIlet::init(Clock * _clk_instance, ManyCoreArch * _manycore_ptr)
{
...
void *SequenceIlet::generate(void *obj)
{
    ...
    while(1)
    {
        ...
        {
            ILet * new_ilet = new ILet(NORMAL, i);
            new_ilet->add_operation(INVADE, 1);
            new_ilet->add_operation(INFECT, 100);
