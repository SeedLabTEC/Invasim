 ...
 * @param _clk_instance 
 */
void ManyCoreArch::init(int _x_dim, int _y_dim, Clock *_clk_instance, InterconnectionNetwork*& _intNet)
{
    ...
    for (int i = 0; i < _x_dim; i++)
    {
        ...
        {
            this->pu_array[i][j] = new ProcessingUnit(i, j, _clk_instance, _intNet, this->ptr_registers);
        }
    }
